  ...
  const getAllOrders = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:8000/api/v1/product/all-orders"
      );
  ...
  const handleStatusChange = async (value, orderId) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:8000/api/v1/product/order-status-update/${orderId}`,
        { status: value }
      );
                      ...
                      {ord.products.map((pd, i) => (
                        <div className="row m-1 mb-2 p-3 card flex-row" key={i}>
                          <div className="col md-4">
                            <img
                              src={`http://localhost:8000/api/v1/product/product-photo/${pd._id}`}
